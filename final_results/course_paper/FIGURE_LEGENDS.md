...
**Figure 7: Net launches are a recurring but minority capture mode.** **(A)** Adjacent and net-launcher capture actions reconstructed from all per-turn logs in the 1,080-game main and deck suite (7,155 captures; 1,367 net launches). Counts are events, not games, and therefore reflect both opportunity and agent behavior. **(B)** Net launches as a percentage of each agent's captures, with exact percentages printed beside the bars. Expectimax depth 4 is excluded because it appears only in the smaller bounded feasibility study. Across the displayed agents, the net share ranges from 9.5% (Rule) to 23.5% (Expectimax d2).
