...
- **Custom boards.** Boards defined in `boards/*.json` (e.g. `crossfire`) may replace the single center tile with several weighted **capture cells**, each worth its own points at the end of your turn, and may add blocked cells; the default board has just the 1-point center. All center rules above apply to each capture cell.
